...
import { generateBillingPeriods, allocateDueStatus, findClassDoc, isPaymentVerified } from "./fee-utils.js";
...
function buildUpiLink({ upiId, payeeName, amount, note }) {
  const params = new URLSearchParams({
    pa: upiId,
    pn: payeeName || "Home Tuition Classes",
    am: String(amount),
    cu: "INR",
    tn: note || "Tuition Fee"
  });
...
async function init() {
  ...
  if (!feeAmount) {
    renderState("fa-triangle-exclamation", "Is class ke liye fee abhi set nahi hui hai. Sampark karein.");
    return;
  }
  ...
  if (alreadyPendingAmount >= period.amountDue) {
    content.innerHTML = `
      <div class="pay-card">
        ...
        <div class="pay-who">${escapeHtml(student.name || roll)} · Roll ${escapeHtml(roll)}</div>
        <div class="pay-period">${escapeHtml(period.label)}</div>
        <p class="pay-confirm-hint">Aap is period ka payment pehle hi report kar chuke hain. Admin verify karne ke baad yeh "Paid" dikhega.</p>
        <a href="parent-dashboard.html" class="pay-back" style="justify-content:center;">
          <i class="fa-solid fa-arrow-left"></i> Dashboard par jaayein
        </a>
      </div>
  ...
  let upiId = "";
  let payeeName = "Home Tuition Classes";
  try {
  ...
  if (!upiId) {
    content.innerHTML = `
      <div class="pay-card">
        ...
        <p class="pay-confirm-hint">Online (UPI) payment abhi set up nahi hua hai. Kripya sampark karke payment karein.</p>
      </div>
    `;
    return;
  }
  ...
  content.innerHTML = `
    <div class="pay-card">
      ...
      <p class="pay-confirm-hint">
        ...
        <strong>admin verify karega</strong> (apne bank/UPI statement se match karke), uske baad hi status "Paid" mein badlega.
      </p>
    ...
    try {
      ...
      content.innerHTML = `
        <div class="pay-card">
          ...
          <p class="pay-confirm-hint">${escapeHtml(period.label)} ka ₹${amountDue} payment report ho gaya hai. Admin verify karne ke baad yeh "Paid" dikhega. Dashboard par redirect kiya ja raha hai...</p>
        </div>
      `;
