...
import { generateBillingPeriods, allocateDueStatus, findClassDoc, classKeyOf, isPaymentVerified } from "./fee-utils.js";
...
// classKeyOf() and findClassDoc() live in fee-utils.js (shared
// with pay-fee.js, which needs the exact same class -> fee-
// structure lookup when independently re-verifying an amount
// before writing a payment).
...
// Portal "separate from student login", and it doubles as the
// key used to find every sibling registered under the same
// contact number.
// ==========================================================
async function parentLogin() {
  ...
  try {
    ...
    if (!storedContact) {
      alert("Is Roll Number ke liye koi mobile number register nahi hai. Kripya sampark karein.");
      return;
    }
...
// a switcher for parents with more than one child.
// ==========================================================
...
async function loadChildAttendance(roll, month) {
  ...
  try {
    // Fetch the whole attendance subcollection and filter/sort
    // client-side (avoids Firestore needing a composite index for
    // a range-filter + orderBy on documentId()). A student's
    // attendance subcollection is small (one doc per school day),
    // so this stays cheap.
...
// same one notices.html on the admin side writes to — same for
// every child regardless of which one is active)
// ==========================================================
...
// ==========================================================
// Fee Status — month-by-month due, anchored to admission date.
// Only VERIFIED payments count toward "Paid"; a self-reported
// UPI payment from the Pay Now flow sits as "Pending
// Verification" until admin confirms it.
// ==========================================================
  ...
  try {
    ...
    if (dueBox) {
      if (!feeAmount) {
        ...
        dueBox.innerHTML = `<p class="result-empty-msg">Admission Date set nahi hai, isliye month-wise due nahi dikhaya ja sakta. Sampark karein.</p>`;
      } else {
